...
from .encoder_layer import VisionLanguageModelOutput,DeformableDetrEncoderLayer
from .vl_bi_encoder import BiAttentionBlock
from einops.layers.torch import Rearrange 
from transformers.pytorch_utils import meshgrid

class MultiScaleCrossModalEncoder(nn.Module):
    def __init__(self, config: Mono3DVGv2Config):
        super().__init__()
        self.config = config
        
        self.vision_input_adapter = nn.Sequential(
            Rearrange('b (h w) c -> b c h w', h=int(math.sqrt(config.vision_position_embeddings)))  # 假设序列长度是平方数
        )
        
        self.text_input_adapter = nn.Sequential(
            Rearrange('b (h w) c -> b c h w', h=int(math.sqrt(config.text_position_embeddings)))  # 假设序列长度是平方数
        )
        
        self.vision_output_adapter = nn.Sequential(
            nn.Conv2d(in_channels=8, out_channels=256, kernel_size=1, stride=1, padding=0),
            Rearrange('b c h w -> b (h w) c')  # 假设序列长度是平方数
        )
        
        self.text_output_adapter = nn.Sequential(
            Rearrange('b c h w -> b (h w) c')  # 假设序列长度是平方数
        )     
        ...
        # 多尺度跨模态模块
        self.mhcsab = MHCSAB()
        self.drm = DictionaryRepresentationModule()
        self.fusion_moudle = nn.LayerNorm(self.config.d_model)
    ...
    def forward(
        ...
        if isinstance(text_embeds, (list, tuple)):
            # 如果文本嵌入是列表/元组，取最后一层的输出
            text_embeds = text_embeds[0]        
        all_attentions = () if output_attentions else None
        # text_input = self.text_input_adapter(text_embeds)
        visual_input = self.vision_input_adapter(vision_embeds)  # [batch, 10, H, W]
        # _, _, h, w = visual_input.size()
        # visual_input = F.interpolate(visual_input, (int(h // 16) * 16, int(w // 16) * 16), mode="bilinear", align_corners=False)
        
        visual_features = self.visual_backbone(visual_input)
        
        vis_dict, _ = self.drm(visual_features)
        
        # text_dict, _ = self.drm(text_input)

        # print(f"fused_visual shape: {torch.cat([vis_dict,visual_features],dim=1).shape}") 
        # fused_text = self.mhcsab(text_dict)
        # vis_dict = self.mhcsab(vis_dict)
        vis_dict = self.vision_output_adapter(vis_dict)
        # print(f"dict_features shape: {vis_dict.shape}")
        # print(f"visual_features shape: {vision_embeds.shape}")        
        fused_visual = self.fusion_moudle(vis_dict+vision_embeds)
        # fused_visual = F.interpolate(fused_visual, (int (h) , int (w)), mode="bilinear", align_corners=False)
        # fused_text = F.interpolate(fused_text, (10, 11), mode="bilinear", align_corners=False)     
        
        # fused_text = self.text_output_adapter(text_dict)
        fused_text = text_embeds

        reference_points = get_reference_points(
            spatial_shapes, valid_ratios, device=fused_visual.device
        )

        for layer_id, layer in enumerate(self.layers):
            if self.cross_attn:
                fused_visual, fused_text = self.cross_attn[layer_id](
                    v=fused_visual,
                    l=fused_text,
            ...
            fused_visual = layer_outputs[0]
            if output_attentions:
                all_attentions = all_attentions + (layer_outputs[1],)            
            
        if not return_dict:
            return tuple(v for v in [fused_visual, fused_text, all_attentions] if v is not None)
        return VisionLanguageModelOutput(
            last_vision_hidden_state=fused_visual,
            last_text_hidden_state=fused_text,
            attentions=all_attentions,
        )

    def _spatial_align(self, text_feats, visual_feats):
        """将文本特征对齐到视觉特征空间"""
        batch, seq_len, dim = text_feats.shape
        h, w = visual_feats.shape[2:]
        
        # 方法1：空间展开+线性投影
        text_feats = text_feats.reshape(batch, seq_len//(h*w), h, w, dim)
        return text_feats.mean(1).permute(0,3,1,2)
        
        # 方法2：位置敏感投影
        # pos_emb = position_embeddings.expand(batch, -1, -1, -1)
        # return torch.einsum('bchw,bthw->bcthw', pos_emb, text_feats).sum(-1)
...
class base(nn.Module):
    def __init__(self):
        ...
        self.B = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ReflectionPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=64, out_channels=16, kernel_size=(3, 3), stride=1),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.ReflectionPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=16, out_channels=8, kernel_size=(3, 3), stride=1),
...
class FusionMoudle(nn.Module):
    def __init__(self):
        ...
        self.D = nn.Sequential(
            nn.ReflectionPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=16, out_channels=64, kernel_size=(3, 3), stride=1),
            nn.BatchNorm2d(64),
            nn.PReLU(),
            nn.ReflectionPad2d((1, 1, 1, 1)),
            ...
            nn.PReLU(),
            nn.ReflectionPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=64, out_channels=256, kernel_size=(3, 3), stride=1),
            nn.BatchNorm2d(256),
            nn.PReLU(),
        )
...
class MHCSAB(nn.Module):
    ...
    def forward(self, input):
        ...
        pad_h = (self.largesize - (h % self.largesize)) % self.largesize
        pad_w = (self.largesize - (w % self.largesize)) % self.largesize
        input_padded = F.pad(input, (0, pad_w, 0, pad_h))
        
        window_size = input.size()[3]
        
        new_h = input_padded.size(2)
        new_w = input_padded.size(3)
        flod_win_l = nn.Fold(
        ...
        flod_win_s = nn.Fold(output_size=(new_h, new_w), kernel_size=(self.smallsize, self.smallsize),
                             stride=self.smallsize).to(input.device)
        ...
        large_scale_f = self.LargeScaleEncoder(input_padded)
        small_scale_f = self.SmallScaleEncoder(input_padded)
