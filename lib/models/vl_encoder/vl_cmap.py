...
class MultiScaleCrossModalEncoder(nn.Module):
    def __init__(self, config: Mono3DVGv2Config):
        ...
        self.config = config
            ...
            # 跨模态融合组件
        self.cross_attn = BiAttentionBlock(
    ...
    def forward(
        ...
        vis = vision_embeds
        ir = text_embeds
        # print(f"fused_visual形状: {vision_embeds.shape}")
        # print(f"visual_features形状: {text_embeds[0].shape}")
        # print(f"visual_features形状: {text_embeds[1].shape}")
        # print(f"visual_features形状: {text_embeds[2].shape}")
        # ir = self._spatial_align(self, text_embeds[0], vision_embeds)
        
        
        with torch.no_grad():
            vis_d, ir_d, _, index_r, _ = ImageTransform()(vis, ir)
        
        vis_1 = base(vis)
        ir_d_1 = base(ir_d)

        
        vis_f = FeatureExtractor(vis_1)
        ir_d_f = FeatureExtractor(ir_d_1)

        
        vis_e_f = Enhance(vis_f)
        ir_d_e_f = Enhance(ir_d_f)

        VISDP_vis_f, _ = DictionaryRepresentationModule(vis_e_f)
        IRDP_ir_d_f, _ = DictionaryRepresentationModule(ir_d_e_f)
        
        fixed_DP = VISDP_vis_f
        moving_DP = IRDP_ir_d_f
        
        moving_DP_lw = df_window_partition(moving_DP, 26, 16)
        fixed_DP_sw = window_partition(fixed_DP, 16, 26)
        
        fused_visual, fused_text = self.cross_attn(fixed_DP_sw,moving_DP_lw)
