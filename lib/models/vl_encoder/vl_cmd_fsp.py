...
class MultiScaleCrossModalEncoder(nn.Module):
    ...
    def forward(
        ...
        fused_visual = self.mhcsab(vis_dict)
        # print(f"dict_features shape: {dict_features.shape}")
        # print(f"visual_features shape: {visual_features.shape}")
        # print(f"fused_visual shape: {fused_visual.shape}")        
        ...
        fused_visual = self.vision_output_adapter(fused_visual)
        fused_text = self.text_output_adapter(fused_text)

        for layer_id, layer in enumerate(self.cross_attn):
            fused_visual, fused_text = layer(
                v=fused_visual,
                l=fused_text,
        ...
        if not return_dict:
            return (fused_visual, fused_text)
...
class MHCSAB(nn.Module):
    def __init__(self):
        ...
        self.LargeScaleEncoder = nn.Sequential(
            ...
            nn.PReLU(),
            nn.ReflectionPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=64, out_channels=4, kernel_size=(3, 3), stride=1),
            nn.InstanceNorm2d(4),
            nn.PReLU(),
        )
